# Vendored on 2022-02-21 from master
"""
...
# * Designed to be run inside of an asyncio loop
# * Support for coroutines
# * Support for one-time jobs
...
class Scheduler:
    ...
    def in_(self, interval: int = 1) -> "OneTimeJob_Relative":
        """
        Schedule a one-time job (in a literate way)
        """
        return OneTimeJob_Relative(interval, self)

    def at(self, time: int | datetime.datetime) -> "OneTimeJob_Absolute":
        """
        Schedule a one-time job (in a data way)
        """
...
class BaseJob:
    """
    Common bits between recurring and one-time jobs.

    Represents the minimum interface needed for a job.
    """
    ...
    def do(self, job_func: Callable, *args, **kwargs):
        ...
        :return: The invoked job instance
        """
        # FIXME: Make sure exceptions are caught and logged
        if asyncio.iscoroutinefunction(job_func):
            self.job_func = lambda: asyncio.create_task(
...
class OneTimeMixin(BaseJob):
    """
    Stuff on top of BaseJob to implement one-time jobs.
    """
    # Uses last_run as a "has run" indicator
    @property
    def should_run(self) -> bool:
        """
        :return: ``True`` if the job should be run now.
        """
        return self.last_run is not None and super().should_run
...
class OneTimeJob_Relative(OneTimeMixin, BaseJob):
    """
    A one-time job scheduled relative to now
    ...
    * a quantity of `time units` defined by `interval`
    ...
    A job is usually created and returned by :meth:`Scheduler.in_`
    method, which also defines its `interval`.
    """
...
class OneTimeJob_Absolute(OneTimeMixin, BaseJob):
    def __init__(self, when: datetime.datetime, scheduler: Scheduler = None):
        super().__init__(scheduler)
        self.next_run = when
    ...
    def __repr__(self) -> str:
        ...
        return ("{}(at={}, do={}, args={}, kwargs={})").format(
            type(self).__name__,
            self.when,
            job_func_name,
            "()" if self.job_func is None else self.job_func.args,
    ...
    def _schedule_next_run(self) -> None:
        ...
        Compute the instant when this job should run next.
        """
        # This is already handled by __init__
...
def in_(interval):
    return default_scheduler.in_(interval)
